...
- **Categories are a closed vocabulary.** Don't accept arbitrary strings into `Project.category` or `SpecialistProfile.category` — always pass through `clamp_category()` (backend) or `clampCategory()` (frontend). Extending the list requires editing **both** `backend/app/core/categories.py` and `frontend/src/lib/categories.ts` in the same change.
- **Specialist feed scoping is implicit.** `GET /api/v1/projects` infers the category filter from the caller's profile when they're an authenticated specialist — there is no `?category=` query param to override it. Test paths that need a different scope must use a different role or no auth.
- **`record_view` is best-effort.** Catch nothing inside it; let it swallow its own errors. Callers must not depend on it succeeding, and view counts are not part of any consistency invariant.
...
| Extend the category vocabulary                | `backend/app/core/categories.py` **and** `frontend/src/lib/categories.ts` — both, same commit |
| Tune the keyword classifier                   | `KEYWORDS` table in `backend/app/core/categories.py` (mirror in `frontend/src/lib/categories.ts`) |
| Change feed filter behavior                   | `backend/app/repositories/projects.py::list_open_projects` + router signature in `backend/app/api/v1/projects.py::public_feed` |
| Add a new feed sort mode                      | Extend `SortMode` literal in `repositories/projects.py`, add the branch, mirror `FeedSort` in `frontend/src/features/projects/api.ts` |
| Track a new per-user event like view-tracking | Model in `backend/app/models/`, service in `backend/app/services/` wrapped in `try/except`, call site at the appropriate endpoint |
